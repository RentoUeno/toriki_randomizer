...
// 全アイテム共通の音声候補（必要に応じてアイテムごとに上書きも可）
const ALL_SOUNDS = [
  "sound1",
  "sound2",
  "sound4",
...
const ITEM_STYLES = [
  ...
  },
  {
    match:    "黒霧島（芋）",
    cls:      "kirishima",
    soundIds: ALL_SOUNDS,   // kirishima専用にしたいなら ["kirishima_sound_1", ...] に変える
    volume:   1.0,
  },
  {
    match:    "大隅（麦）",
...
function buildAlcoholPool() {
  ...
  if (document.getElementById("mega")?.checked)      pool.push("☠☠☠メガ豪快☠☠☠");
  if (document.getElementById("premium")?.checked)   pool.push("☠☠プレミアム豪快☠☠");
  if (document.getElementById("mixjuice")?.checked)  pool.push("ミックスジュース");
  return pool;
}

/**
 * チェックボックスの値をまとめて取得するヘルパー
 */
...
function goukai_pick() {
  ...
  for (let i = 0; i < count; i++) {
    ...
    result.appendChild(createResultItem(item));
  }
}
...
function goukai() {
  ...
  if (getCheckboxValue("alcohol")) {
    extra.innerHTML = `
      <br>
      <label>
        <input type="checkbox" id="mega">メガ豪快を含む
      </label>
      <br><br>
      <label>
        <input type="checkbox" id="premium">プレミアム豪快を含む
      </label>
      <br><br>
      <label>
        <input type="checkbox" id="mixjuice">ミックスジュースを含む
      </label>
      <br><br>
      <label>
